...
def test_login_command_saves_credentials(tmp_path):
    ...
    with (
        patch("insighta.auth.CREDENTIALS_PATH", fake_path),
        patch(
            "insighta.auth.generate_pkce_pair", return_value=("verifier", "challenge")
        ),
...
def test_logout_clears_credentials(tmp_path):
    ...
    fake_path.write_text(
        '{"access_token": "abc", "refresh_token": "xyz", "username": "danielpopoola"}'
    )

    with (
        patch("insighta.auth.CREDENTIALS_PATH", fake_path),
        patch(
            "insighta.http.load_credentials",
            return_value={
                "access_token": "abc",
                "refresh_token": "xyz",
